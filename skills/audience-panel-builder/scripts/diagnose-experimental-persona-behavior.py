#!/usr/bin/env python3
"""Diagnose one frozen synthetic persona-behavior evidence projection."""
...
def _read_input(path: Path) -> dict[str, object]:
    ...
    if not isinstance(value, dict) or set(value) != _INPUT_KEYS:
        raise ContractError("input must be the closed diagnosis request")
    return value
...
def _arguments(args: argparse.Namespace) -> dict[str, object]:
    ...
    if missing:
        raise ContractError(
            "staged diagnosis arguments are incomplete: " + ", ".join(missing)
        )
    return {
        key: (
            _read_value(value)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--diagnosed-at")
    output = parser.add_mutually_exclusive_group(required=True)
    output.add_argument("--output", type=Path)
    output.add_argument("--private-stage-output", type=Path)
    args = parser.parse_args(argv)
    try:
        result = diagnose_persona_behavior(**_arguments(args))
